...
function keteranganki1() {
    return Keterangan.filter((element) => element.dokumen == "KI-1")
}
...
function DocKI1() {
    ...
    const onSubmit = (dataSubmit) => {
        ...
        if(dataCekKom.length == 0) {
            const response = modelPelepasan.eksporKI(dataSubmit);
            response
            .then((response) => {
                setOnLoad(false)
                if(response.data) {
                    if(response.data.status == 201) {
                        //start save history
                        const resHsy = log.pushHistory(dataSubmit.idPtk, "p8", "KI-1", (dataSubmit.idDoki1 ? 'UPDATE' : 'NEW'));
                        resHsy
                        .then((response) => {
                            if(response.data.status == 201) {
                        ...
                        Swal.fire({
                            title: "Sukses!",
                            text: "HC Ikan dan Produk Ikan berhasil " + (dataSubmit.idDoki1 ? "diedit." : "disimpan."),
                            icon: "success"
                        });
                        setValue("idDoki1", response.data.data.id)
                        setValue("noDoki1", response.data.data.nomor)
                    } else {
                        Swal.fire({
                            title: "Error!",
    ...
    const {
        register: registerMPki1,
        setValue: setValueMPki1,
        // control: controlMPki1,
        watch: watchMPki1,
        handleSubmit: handleFormMPki1,
        reset: resetFormKomoditiki1,
        formState: { errors: errorsMPki1 },
    } = useForm({
        defaultValues: {
            idMPki1: "",
            volumeNetto: "",
    ...
    const cekdataMPki1 = watchMPki1()

    function onSubmitMPki1(data) {
        setOnLoad(true)
        let cekVolume = false
        ...
        if(cekVolume) {
            log.updateKomoditiP8(data.idMPki1, data)
            .then((response) => {
                setOnLoad(false)
                if(response.data.status == 201) {
                    Swal.fire({
    ...
    function handleEditKomoditas(e) {
        ...
        setValueMPki1("idMPki1", dataMP[0].id)
        setValueMPki1("idPtk", dataMP[0].ptk_id)
        setValueMPki1("jenisKar", "I")
        ...
        setValueMPki1("nettoP8", dataMP[0].volume_netto)
        setValueMPki1("satuanNetto", dataMP[0].sat_netto)
        setValueMPki1("volumeP8", dataMP[0].volume_lain)
        setValueMPki1("satuanLain", dataMP[0].sat_lain)
        setValueMPki1("namaUmum", dataMP[0].nama_umum_tercetak)
        setValueMPki1("namaLatin", dataMP[0].nama_latin_tercetak)
    }
   ...
   function refreshListKomoditas() {
    ...
    useEffect(()=>{
        if(idPtk) {
            setValue("tglDoki1", (new Date()).toLocaleString('en-CA', { hourCycle: 'h24' }).replace(',', '').slice(0,16))
                ...
                if(response.data) {
                    if(typeof response.data != "string") {
                        ...
                        if(response.data.status == 200) {
                            setValue("idDoki1", response.data.data.id)
                            setValue("noDoki1", response.data.data.nomor)
                            setValue("tglDoki1", response.data.data.tanggal)
                        ...
                        setData(values => ({...values,
                            errorki1: "Gagal load data HC Ikan dan Produk Ikan"
                        }));
                    }
                }
            })
    ...
    <div className="container-xxl flex-grow-1 container-p-y">
        <h4 className="py-3 breadcrumb-wrapper mb-4">
            ...
            <small className='float-end'>
                <span className='text-danger'>{(data.errorPTK ? data.errorPTK + "; " : "") + (data.errorKomoditas ? data.errorKomoditas + "; " : "") + (data.errorki1 ? data.errorki1 + "; " : "") + (data.errorPeriksaFisik ? data.errorPeriksaFisik + "; " : "") + (data.errorSurtug ? data.errorSurtug + "; " : "")}</span>
                {data.errorPTK || data.errorKomoditas || data.errorki1 || data.errorPeriksaFisik || data.errorSurtug ?
            ...
            <div className="col-xxl">
                <div className="card card-action mb-4">
                    ...
                    <form className="card-body" onSubmit={handleSubmit(onSubmit)}>
                        <input type="hidden" id='idDoki1' {...register("idDoki1")} />
                        ...
                        <div className="col-md-12">
                            <div className="row mb-3">
                                <label className="col-sm-2 col-form-label text-sm-start" htmlFor="noDoki1">Nomor Dokumen</label>
                                <div className="col-sm-3">
                                    <input type="text" id="noDoki1" name='noDoki1' {...register("noDoki1")} className="form-control form-control-sm" placeholder="Nomor Dokumen KI-1" disabled />
                                </div>
                                <label className="col-sm-3 col-form-label text-sm-end" htmlFor="tglDoki1">Tanggal <span className='text-danger'>*</span></label>
                                <div className="col-sm-2">
                                    <input type="datetime-local" id="tglDoki1" name='tglDoki1' {...register("tglDoki1", {required: "Mohon isi tanggal dokumen."})} className={errors.tglDoki1 ? "form-control form-control-sm is-invalid" : "form-control form-control-sm"} />
                                    {errors.tglDoki1 && <small className="text-danger">{errors.tglDoki1.message}</small>}
                                </div>
                            </div>
                        </div>
                        <div className="accordion mb-4" id="collapseSection">
                                ...
                                <div id="collapseKeterangan">
                                    <div className="accordion-body">
                                        ...
                                        <div className="row">
                                            <div className="col-md-6">
                                                <div className="row">
                                                    <label className="col-sm-4 col-form-label" htmlFor="tujuanPengiriman">Diperuntukan sebagai</label>
                                                    <div className="col-sm-8">
                                        ...
                                        <div className="row mb-3">
                                            <div className='col-md-6'>
                                                <div className="row">
                                                    <label className="col-sm-4 col-form-label" htmlFor="namaLabUji">Laboratorium Penguji</label>
                                                    <div className="col-sm-8">
                                                        <input type="text" id="namaLabUji" name='namaLabUji' {...register("namaLabUji")} className="form-control form-control-sm" placeholder="Nama Laboratorium Penguji" />
                                                    </div>
                                                    <label className="col-sm-4 col-form-label" htmlFor="alamatLabUji">Alamat Laboratorium</label>
                                                    <div className="col-sm-8">
                                                        <textarea name="alamatLabUji" id="alamatLabUji" {...register("alamatLabUji")} placeholder="Alamat Laboratorium Penguji" className="form-control form-control-sm" rows="2" {...register("alamatLabUji")}></textarea>
                                                    </div>
                                                </div>
                                            </div>
                                            <div className="col-sm-6">
                                                <div className="row">
                                                    <label className="col-sm-4 col-form-label" htmlFor="penanggungjawab_lab">Penanggungjawab Laboratorium</label>
                                                    <div className="col-sm-8">
                                                        <input type="text" id="penanggungjawab_lab" className="form-control form-control-sm" placeholder="Penanggungjawab Laboratorium" />
                                                    </div>
                                                    <label className="col-sm-4 col-form-label" htmlFor="no_hasil_lab">Nomor laporan hasil uji</label>
                                                    <div className="col-sm-8">
                            ...
                            <div className="card">
                                <h2 className="accordion-header" id="headerPerlakuan">
                                    <button className="accordion-button" type="button" style={{backgroundColor: '#123138'}} data-bs-toggle="collapse" data-bs-target="#collapsePerlakuan" aria-expanded="true" aria-controls="collapseImporter">
                                        <h5 className='text-lightest mb-0'>III. Pengesahan</h5>
                                    </button>
                                </h2>
                                <div id="collapsePerlakuan">
                                    <div className="accordion-body">
                                        <div className="row">
                                            <div className="col-md-12">
                                                <div className="row mb-3">
                                                    <label className="col-sm-2 col-form-label" htmlFor="no_hasil_lab">Menyatakan Bahwa :</label>
                                                    <div className="col-sm-6">
                                                        <div className="form-check form-check-inline">
                                                        ...
                                                        <div className="form-check form-check-inline">
                                                            <input className="form-check-input" type="checkbox" value="" id="jenis_kom5" />
                                                            <label className="form-check-label" htmlFor="jenis_kom5">
                                                                Lainnya
                                                            </label>
                                                        </div>
                                                    </div>
                                                </div>
                                                {keteranganki1().map((data,index) => (
                                                    <div className="form-check" key={index}>
                                                        <label className="form-check-label" htmlFor={'attestation' + (index+1)}>{data.deskripsi}</label>
                                                        <input className="form-check-input" type="checkbox" {...register('attestation' + (index + 1))} name={'attestation' + (index + 1)} value="1" id={'attestation' + (index + 1)} />
                                                        
                                                        {data.id == 24 ?
                                                            <input style={{display: (cekWatch.attestation5 == "1" ? "block" : "none")}} type="text" {...register('attestationLain')} placeholder='Lainnya..' className='w-50 form-control form-control-sm' />
                                                        : ""}
                                                        {data.id == 22 ? 
                                                        <>
                                                            <div className="form-check" style={{marginLeft: '20px'}}>
                                                                <input className="form-check-input" type="checkbox" value="1" name='attestationFreeFrom' id="attestationFreeFrom" {...register('attestationFreeFrom')} />
                                                                <label className="form-check-label" htmlFor="attestationFreeFrom">
                                                                    bebas dari penyakit;
                                                                </label>
                                                                <input type="text" style={{display: (cekWatch.attestationFreeFrom == "1" ? "block" : "none")}} className='w-50 form-control form-control-sm' {...register('attestationFreeFromText')} />
                                                            </div>
                                                            <div className="form-check" style={{marginLeft: '20px'}}>
                                                                <input className="form-check-input" type="checkbox" value="1" name='attestationNoSign' id="attestationNoSign" {...register('attestationNoSign')} />
                                                                <label className="form-check-label" htmlFor="attestationNoSign">
                                                                    tidak menunjukkan gejala penyakit secara visual/klinis;
                                                                </label>
                                                                <input type="text" style={{display: (cekWatch.attestationNoSign == "1" ? "block" : "none")}} className='w-50 form-control form-control-sm' {...register('attestationNoSignText')} />
                                                            </div>
                                                        </> : ""}
                                                    </div>
                                                ))}
                                                <hr />
                                                <div className="row mb-3">
                                                    <label className="col-sm-2 col-form-label" htmlFor="addInfo">Informasi Tambahan</label>
                                                    <div className="col-sm-6">
                                                        <textarea name="addInfo" id="addInfo" rows="2" {...register("addInfo")} className='form-control form-control-sm'></textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            <div className="modal-dialog modal-lg">
                <div className="modal-content p-3 pb-1">
                    <div className="modal-body">
                        ...
                        <form onSubmit={handleFormMPki1(onSubmitMPki1)} className="row g-3">
                        <input type="hidden" name='idMPki1' {...registerMPki1("idMPki1")} />
                        <input type="hidden" name='idPtk' {...registerMPki1("idPtk")} />
                        <input type="hidden" name='jenisKar' {...registerMPki1("jenisKar")} />
                            <div className="col-6">
                                <label className="form-label" htmlFor="namaUmum">Nama Umum Tercetak</label>
                                <input type='text' name="namaUmum" id="namaUmum" {...registerMPki1("namaUmum")} className="form-control form-control-sm" />
                            </div>
                            <div className="col-6">
                                <label className="form-label" htmlFor="namaLatin">Nama Latin Tercetak</label>
                                <input type='text' name="namaLatin" id="namaLatin" {...registerMPki1("namaLatin")} className="form-control form-control-sm" />
                            </div>
                            <div className="col-6">
                                ...
                                <div className='row'>
                                    <div className="col-5" style={{paddingRight: '2px'}}>
                                        <input type="text" name='nettoP8' id='nettoP8' value={cekdataMPki1.nettoP8 ? addCommas(removeNonNumeric(cekdataMPki1.nettoP8)) : ""} {...registerMPki1("nettoP8", {required: "Mohon isi volume netto."})} className={errorsMPki1.nettoP8 ? "form-control form-control-sm is-invalid" : "form-control form-control-sm"} />
                                    </div>
                                    <div className="col-3" style={{paddingLeft: '2px'}}>
                                        <input type="text" className='form-control form-control-sm' name='satuanNetto' id='satuanNetto' {...registerMPki1("satuanNetto")} disabled />
                                    </div>
                                </div>
                                {errorsMPki1.volumeNetto && <small className="text-danger">{errorsMPki1.volumeNetto.message}</small>}
                            </div>
                            <div className="col-6">
                                <label className="form-label" htmlFor="volumeP8">Volume Lain Akhir-P8</label>
                                <div className='row'>
                                    <div className="col-5" style={{paddingRight: '2px'}}>
                                        <input type="text" className='form-control form-control-sm' name='volumeP8' id='volumeP8' value={cekdataMPki1.volumeP8 ? addCommas(removeNonNumeric(cekdataMPki1.volumeP8)) : ""} {...registerMPki1("volumeP8")} />
                                    </div>
                                    <div className="col-3" style={{paddingLeft: '2px'}}>
                                        <input type="text" className='form-control form-control-sm' name='satuanLain' id='satuanLain' {...registerMPki1("satuanLain")} disabled />
                                    </div>
                                </div>
                            </div>
