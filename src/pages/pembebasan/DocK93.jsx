...
function DocK93() {
            ...
            .then((response) => {
                ...
                if(response.data) {
                    if(response.data.status == 201) {
                        ...
                        setValue("noDok93", response.data.data.nomor)
                    } else {
                        Swal.fire({
                            title: "Error!",
    ...
    const {
        register: registerMPk93,
        setValue: setValueMPk93,
        // control: controlMPk93,
        watch: watchMPk93,
        handleSubmit: handleFormMPk93,
        reset: resetFormKomoditik93,
        formState: { errors: errorsMPk93 },
    } = useForm({
        defaultValues: {
            idMPk93: "",
            volumeNetto: "",
    ...
    const cekdataMPk93 = watchMPk93()

    function onSubmitMPk93(data) {
        setOnLoad(true)
        let cekVolume = false
        ...
        if(cekVolume) {
            log.updateKomoditiP8(data.idMPk93, data)
            .then((response) => {
                setOnLoad(false)
                if(response.data.status == 201) {
                    Swal.fire({
    ...
    function handleEditKomoditas(e) {
        ...
        setValueMPk93("idMPk93", dataMP[0].id)
        setValueMPk93("idPtk", dataMP[0].ptk_id)
        setValueMPk93("jenisKar", "I")
        ...
        setValueMPk93("nettoP8", dataMP[0].volume_netto)
        setValueMPk93("satuanNetto", dataMP[0].sat_netto)
        setValueMPk93("volumeP8", dataMP[0].volume_lain)
        setValueMPk93("satuanLain", dataMP[0].sat_lain)
        setValueMPk93("namaUmum", dataMP[0].nama_umum_tercetak)
        setValueMPk93("namaLatin", dataMP[0].nama_latin_tercetak)
    }
   ...
   function refreshListKomoditas() {
    ...
    useEffect(()=>{
        if(idPtk) {
            setValue("tglDok93", (new Date()).toLocaleString('en-CA', { hourCycle: 'h24' }).replace(',', '').slice(0,16))
                ...
                if(response.data) {
                    if(typeof response.data != "string") {
                        ...
                        if(response.data.status == 200) {
                            setValue("idDok93", response.data.data.id)
                            setValue("noDok93", response.data.data.nomor)
                            setValue("tglDok93", response.data.data.tanggal)
                        ...
                        setData(values => ({...values,
                            errork93: "Gagal load data Surat Keterangan Karantina"
                        }));
                    }
                }
            })
  ...
  return (
    <div className="container-xxl flex-grow-1 container-p-y">
        <h4 className="py-3 breadcrumb-wrapper mb-4">
            K-9.3 <span className="fw-light" style={{color: 'blue'}}>SURAT KETERANGAN KARANTINA</span>

            <small className='float-end'>
                <span className='text-danger'>{(data.errorPTK ? data.errorPTK + "; " : "") + (data.errorKomoditas ? data.errorKomoditas + "; " : "") + (data.errork93 ? data.errork93 + "; " : "") + (data.errorPeriksaFisik ? data.errorPeriksaFisik + "; " : "") + (data.errorSurtug ? data.errorSurtug + "; " : "")}</span>
                {data.errorPTK || data.errorKomoditas || data.errork93 || data.errorPeriksaFisik || data.errorSurtug ?
            ...
            <div className="col-xxl">
                <div className="card card-action mb-4">
                    ...
                    <form className="card-body" onSubmit={handleSubmit(onSubmit)}>
                        <input type="hidden" id='idDok93' {...register("idDok93")} />
                        ...
                        <div className="col-md-12">
                            <div className="row mb-3">
                                <label className="col-sm-2 col-form-label text-sm-start" htmlFor="noDok93">Nomor Dokumen</label>
                                <div className="col-sm-3">
                                    <input type="text" id="noDok93" name='noDok93' {...register("noDok93")} className="form-control form-control-sm" placeholder="Nomor Dokumen K-9.1" disabled />
                                </div>
                                <label className="col-sm-3 col-form-label text-sm-end" htmlFor="tglDok93">Tanggal <span className='text-danger'>*</span></label>
                                <div className="col-sm-2">
                                    <input type="datetime-local" id="tglDok93" name='tglDok93' {...register("tglDok93", {required: "Mohon isi tanggal dokumen."})} className={errors.tglDok93 ? "form-control form-control-sm is-invalid" : "form-control form-control-sm"} />
                                    {errors.tglDok93 && <small className="text-danger">{errors.tglDok93.message}</small>}
                                </div>
                            </div>
                        </div>
                        <div className="accordion mb-4" id="collapseSection">
                            ...
                            <div className="card">
                                <h2 className="accordion-header" id="headerPerlakuan">
                                    <button className="accordion-button" type="button" style={{backgroundColor: '#123138'}} data-bs-toggle="collapse" data-bs-target="#collapsePerlakuan" aria-expanded="true" aria-controls="collapseImporter">
                                        <h5 className='text-lightest mb-0'>III. Keterangan Media Pembawa</h5>
                                    </button>
                                </h2>
                                <div id="collapsePerlakuan">
                                    <div className="accordion-body">
                        ...
                        <div className="pt-2">
                            <div className="row">
                                <div className="offset-sm-2 col-sm-9">
                                    ...
                                    <button type="button" style={{display: (cekWatch.idDok93 ? "block" : "none")}} className="float-end btn btn-info btn-label-secondary"><i className='tf-icons fa-solid fa-paper-plane me-sm-2 me-1'></i> TTE</button>
                                </div>
                            </div>
                        </div>
                    </form>
            ...
            <div className="modal-dialog modal-lg">
                <div className="modal-content p-3 pb-1">
                    <div className="modal-body">
                        ...
                        <form onSubmit={handleFormMPk93(onSubmitMPk93)} className="row g-3">
                        <input type="hidden" name='idMPk93' {...registerMPk93("idMPk93")} />
                        <input type="hidden" name='idPtk' {...registerMPk93("idPtk")} />
                        <input type="hidden" name='jenisKar' {...registerMPk93("jenisKar")} />
                            <div className="col-6">
                                <label className="form-label" htmlFor="namaUmum">Nama Umum Tercetak</label>
                                <input type='text' name="namaUmum" id="namaUmum" {...registerMPk93("namaUmum")} className="form-control form-control-sm" />
                            </div>
                            <div className="col-6">
                                <label className="form-label" htmlFor="namaLatin">Nama Latin Tercetak</label>
                                <input type='text' name="namaLatin" id="namaLatin" {...registerMPk93("namaLatin")} className="form-control form-control-sm" />
                            </div>
                            <div className="col-6">
                                ...
                                <div className='row'>
                                    <div className="col-5" style={{paddingRight: '2px'}}>
                                        <input type="text" name='nettoP8' id='nettoP8' value={cekdataMPk93.nettoP8 ? addCommas(removeNonNumeric(cekdataMPk93.nettoP8)) : ""} {...registerMPk93("nettoP8", {required: "Mohon isi volume netto."})} className={errorsMPk93.nettoP8 ? "form-control form-control-sm is-invalid" : "form-control form-control-sm"} />
                                    </div>
                                    <div className="col-3" style={{paddingLeft: '2px'}}>
                                        <input type="text" className='form-control form-control-sm' name='satuanNetto' id='satuanNetto' {...registerMPk93("satuanNetto")} disabled />
                                    </div>
                                </div>
                                {errorsMPk93.volumeNetto && <small className="text-danger">{errorsMPk93.volumeNetto.message}</small>}
                            </div>
                            <div className="col-6">
                                <label className="form-label" htmlFor="volumeP8">Volume Lain Akhir-P8</label>
                                <div className='row'>
                                    <div className="col-5" style={{paddingRight: '2px'}}>
                                        <input type="text" className='form-control form-control-sm' name='volumeP8' id='volumeP8' value={cekdataMPk93.volumeP8 ? addCommas(removeNonNumeric(cekdataMPk93.volumeP8)) : ""} {...registerMPk93("volumeP8")} />
                                    </div>
                                    <div className="col-3" style={{paddingLeft: '2px'}}>
                                        <input type="text" className='form-control form-control-sm' name='satuanLain' id='satuanLain' {...registerMPk93("satuanLain")} disabled />
                                    </div>
                                </div>
                            </div>
